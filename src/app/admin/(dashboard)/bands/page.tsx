...
export default function AdminBands() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.id) {
      ...
      const res = await supabase.from("bands").update(updateData).eq("id", id);
      error = res.error;
    } else {
      const { id, ...insertData } = formData;
      const res = await supabase.from("bands").insert([insertData]);
      error = res.error;
    }
    ...
    <div className="relative">
      ...
      {isAdding && (
        <div className="bg-card border border-border rounded-xl p-6 mb-8 relative">
          ...
          <h2 className="text-xl font-bold mb-4">{formData.id ? "Edit Peserta Band" : "Tambah Peserta Band"}</h2>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-sm font-medium mb-1">Nama Band</label>
              <input required type="text" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className="w-full px-4 py-2 border border-border rounded-lg bg-background" placeholder="Contoh: The Rockers" />
            </div>
            ...
            <button type="submit" disabled={isSaving} className="bg-ukmred hover:bg-ukmred-dark text-white px-6 py-2 rounded-lg font-medium disabled:opacity-50">
              {isSaving ? "Menyimpan..." : (formData.id ? "Simpan Perubahan" : "Tambahkan Band")}
            </button>
          </form>
        </div>
      )}
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={3} className="p-8 text-center text-foreground/50">Belum ada peserta band yang ditambahkan</td></tr>
              ) : (
